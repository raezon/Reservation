...
//getting the first variable Prix
    ...
    if(prix="on")
        prix=0;
//getting the second variable Type of event
    $.each($(":checkbox:checked[name=Typeofevent]"), function(){
      if($(this).val()!=1)
      Typeofevent.push($(this).val());
    });
    
//getting the third variable Typeofagent
  $.each($(":checkbox:checked[name=Typeofagent]"), function(){
    if($(this).val()!=1)
    Typeofagent.push($(this).val());
  });
  
//getting the fourth variable Numberofagent
  $.each($(":checkbox:checked[name=Numberofagent]"), function(){

    Numberofagent.push($(this).val());
  });
//getting the fifth variable Pricebyhour
  $.each($(":checkbox:checked[name=Pricebyhour]"), function(){
    if($(this).val()!=1)
    Pricebyhour.push($(this).val());
  });
...
//geeting the active element
    ...
    if (
      ...
      $.ajax({
        type: "POST",
        url: "?r=site/filter-security&price="+prix+"&Distance="+distance+"&Typeofevent="+Typeofevent+"&Pricebyhour="+Pricebyhour+"&Distance="+distance+"&hour="+hour+"&Typeofagent="+Typeofagent+"&Numberofagent="+Numberofagent+"&active=" + active,
        timeout: 5000,
        data: {Typeofevent:Typeofevent,Typeofagent:Typeofagent,Numberofagent:Numberofagent,Pricebyhour:Pricebyhour,hour:hour,Distance:distance},
