...
//getting the second variable Type of event
    $.each($(":checkbox:checked[name=photo]"), function(){
      if($(this).val()!=1)
      photo.push($(this).val());
    });
//other
  $.each($(":checkbox:checked[name=other]"), function(){
    if($(this).val()!=1)
    photo.push($(this).val());
  });
...
//geeting the active element
    ...
    if (
      ...
      $.ajax({
        type: "POST",
        url: "?r=site/filter-photo&price=" + prix+"&photo="+photo+"&Distance="+distance+"&other="+other+"&active=" + active,
        
        data: {photo:photo,other:"other",Distance:distance,price:prix},
