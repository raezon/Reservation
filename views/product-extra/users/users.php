...
function getUsers()
{
    ...
    $model = Product::find()
     ...
     $model1 =  ProductParent::find()->where(['partner_id'=>$partner->id])->all();
    ...
    foreach ($model as $value) {
        ...
        if($value->extra!=" "){
            ...
            $model_value[$i]=json_decode($value->extra,true);
...
function updateUser($data, $id)
{
    ...
    foreach ($users as $i => $user) {
        if ($user['id'] == $id) {
            ...
            $model=ProductExtra::find()->andwhere(['id'=>$data['id']])->one();
